...
import transformers
from ureca.logger import WandbPredictionProgressCallback, SaveOnEpochEndCallback
from ureca.dist_utils import init_dist
from ureca.model.internlm2.modeling_internlm2 import InternLM2ForCausalLM
from ureca.model.internvl_chat import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModel)
from ureca.patch import (concat_pad_data_collator,
...
from ureca.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      IMG_CONTEXT_TOKEN, IMG_END_TOKEN,
                                      IMG_START_TOKEN, QUAD_END_TOKEN,
                                      QUAD_START_TOKEN, REF_END_TOKEN,
                                      REF_START_TOKEN,
                                      MASK_CONTEXT_TOKEN, MASK_START_TOKEN, MASK_END_TOKEN)
from ureca.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_mugcap,
                                    preprocess_phi3, decode_coco_rle, build_mask_transform, build_resize_transform,
                                    random_split)
from ureca.train.dataset_packed import PackedDataset, packed_collate_fn
...
class DataTrainingArguments:
    ...
    conv_style: str = field(
        default='mug-cap', metadata={'help': 'Prompt style for a conversation.'}
    )
    meta_path: str = field(
        default=None,
    ...
    # Mask aaded in MUG-CAP
    dynamic_mask_size: bool = field(
        default=False,
        metadata={'help': 'Set to True to use dynamic high resolution strategy for Mask. Default is False.'},
    )
    mask_min_dynamic_patch: int = field(
        default=1,
        metadata={'help': 'The minimum number of dynamic patches. Default is 1.'},
    )
    mask_max_dynamic_patch: int = field(
        default=12,
        metadata={'help': 'The maximum number of dynamic patches. Default is 12.'},
    )
    num_mask_token: int = field(
        default = 8,
        metadata={'help': 'Specify the number of mask token to use. Default is 8.'},
    )
    size_augmentation: bool = field(
        default=False,
        metadata={'help': 'Whether to use size augmentation. Default is False.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f'[Dataset] num_mask_token: {num_mask_token}')
        ...
        self.root = meta['root']
        self.mask_root = meta['mask']
        self.cached_data_dict = {}
        self.tcs_loader = tcs_loader
        self.group_by_length = group_by_length
        self.dynamic_image_size = dynamic_image_size
        self.dynamic_mask_size = dynamic_mask_size
        ...
        # dynamic masking
        if dynamic_mask_size:
            self.mask_min_dynamic_patch = min_dynamic_patch
            self.mask_max_dynamic_patch = max_dynamic_patch

        else:
            self.mask_min_dynamic_patch = 1
            self.mask_max_dynamic_patch = 1

        self.use_mask_thumbnail = use_thumbnail
                ...
                if 'length' in data_item:
                    ...
                    if str_length not in self.conv2length:
                        ...
                        self.conv2length[str_length] = token_length + num_image_token * (
                                ...
                                num_mask_token * self.mask_max_dynamic_patch + self.use_mask_thumbnail) # or  num_mask_token * (max_dynamic_patch + use_thumbnail)
                    else:
    ...
    def get_preprocess_function(self):
        ...
        elif self.template_name == 'mug-cap':
            preprocess_function = preprocess_mugcap
        else:
    ...
    def get_mask_transform(self):
        transform = build_mask_transform(input_size=self.image_size, pad2square=self.pad2square)
        return transform

    def get_resize_transform(self):
        transform = build_resize_transform(input_size=self.image_size, pad2square=self.pad2square)
        return transform

    def get_mask_path(self, image_name):
        mask_path = os.path.join(self.mask_root, f"{image_name}.json")

        return mask_path

    def get_mask(self, mask_path, mask_id):
        with open(mask_path, 'r') as f:
            data = json.loads(f.read())

        masks = data['annotations']
        mask = [mask for mask in masks if mask['id'] == int(mask_id)]

        assert len(mask) == 1, "More than one mask found for id {}".format(mask_id)

        decoded_mask = decode_coco_rle(mask[0]['segmentation'])

        return decoded_mask

    def resize_binary_mask_cv(mask, target_size):
    ...
    def multi_modal_get_item(self, data_item):
        ...
        image_path = self.get_image_path(data_item['image']) # Root + image path

        # Load the image using tcs_loader if available, otherwise use PIL
        image = self.load_image(image_path)

        mask_path = self.get_mask_path(data_item['image'])
        mask = self.get_mask(mask_path, data_item['mask_id'])

        if self.size_augmentation:
            size_transform = self.get_resize_transform()

            image = size_transform(image)
            mask = build_resize_transform(mask, target_size=image.size)

        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
            images = dynamic_preprocess(image, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch,
                                        image_size=self.image_size, use_thumbnail=self.use_thumbnail)
            # TODO: IF dynamic mask processing is needed along with dynamic preprocess same as image
        else:  # Otherwise, use the original image as a single patch
            images = [image]

        if self.dynamic_mask_size:
            mask = Image.fromarray(mask.astype(dtype=np.float64) * 255)
            masks = dynamic_preprocess(mask, min_num=self.mask_min_dynamic_patch, max_num=self.mask_max_dynamic_patch,
                                        image_size=self.image_size, use_thumbnail=self.use_mask_thumbnail, if_mask=True)
        ...
        mask_values = [torch.tensor(mask_transform(mask)) for mask in masks]
        mask_values = torch.stack(mask_values).float().unsqueeze(1) # to float, add channel dim.
        ...
        num_mask_patches = mask_values.size(0)
        if not self.dynamic_mask_size:
            assert num_mask_patches == 1, f'The number of patches should be 1, but got {num_patches}.'
        ...
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  ...
                                  [self.num_mask_token * num_mask_patches], # [self.num_mask_token * num_patches] if dynamic process is applied to the mask
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            dynamic_mask_size=dynamic_mask_size
        )
...
def main():
    ...
    if model_args.use_liger:
        from ureca.patch import apply_liger_kernel_to_internvit
    ...
    if model_args.model_name_or_path is not None:
        ...
        config.dynamic_mask_size = data_args.dynamic_mask_size
        config.mask_min_dynamic_patch = data_args.mask_min_dynamic_patch,
        config.mask_max_dynamic_patch = data_args.mask_max_dynamic_patch

        # Mask token config setting
        config.num_mask_token = data_args.num_mask_token
        ...
        internvl_chat_config = InternVLChatConfig(
            ...
            dynamic_mask_size = data_args.dynamic_mask_size, mask_min_dynamic_patch=data_args.mask_min_dynamic_patch,
            mask_max_dynamic_patch=data_args.mask_max_dynamic_patch,)
    ...
    # Mask token input id
    model.mask_context_token_id = mask_context_token_id
    ...
    train_dataset = build_datasets(
        ...
        max_num_frame=data_args.max_num_frame, num_mask_token=data_args.num_mask_token,
        dynamic_mask_size=data_args.dynamic_mask_size)
    ...
    test_dataset = build_datasets(
        ...
        normalize_type=data_args.normalize_type, min_num_frame=data_args.min_num_frame,
        max_num_frame=data_args.max_num_frame, num_mask_token=data_args.num_mask_token,
        dynamic_mask_size=data_args.dynamic_mask_size, data_type="test")

    # Assuming dataset is a ConcatDataset
    total_size = len(test_dataset)
    test_size = total_size - data_args.testset_size

    # Ensure the dataset has at least 1000 samples
    assert total_size >= data_args.testset_size, "Dataset must have at least 1000 samples."

    # Split the dataset
    _, test_dataset = random_split(test_dataset, [test_size, data_args.testset_size])
    ...
    if model_args.freeze_mask_mlp:
        _freeze_params(model.mask_encoder)
        _freeze_params(model.mask_linear)
    ...
    wandb.init(
        project="mug-cap",
        group=data_args.group_name,  # all runs for the experiment in one group
    )
    ...
    trainer.add_callback(SaveOnEpochEndCallback())
