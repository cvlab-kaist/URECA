...
from torch.utils.data import ConcatDataset, WeightedRandomSampler, random_split, Subset
...
from .constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
                        ...
                        MASK_START_TOKEN, MASK_END_TOKEN, MASK_CONTEXT_TOKEN)
...
def expand2square_mask(np_mask, background_value=0):
    ...
    if width == height:
        ...
        padded_mask = np.pad(np_mask, ((pad_top, pad_bottom), (0, 0)), mode='constant',
                             constant_values=background_value)
    else:
        # Pad width to match height
        pad_left = (height - width) // 2
        pad_right = height - width - pad_left
        padded_mask = np.pad(np_mask, ((0, 0), (pad_left, pad_right)), mode='constant',
                             constant_values=background_value)
...
def build_mask_transform(input_size, pad2square=False):
    def expand2square_mask(mask):
        ...
        pad_h = (size - h) // 2
        pad_w = (size - w) // 2
        padded_mask = np.pad(mask, ((pad_h, size - h - pad_h), (pad_w, size - w - pad_w)), mode='constant', constant_values=0)
        return padded_mask

    def resize_mask(mask):
        # Resize the mask using OpenCV
        return cv2.resize(mask, (input_size, input_size), interpolation=cv2.INTER_NEAREST)

    if pad2square:
        transform = Compose([
            Lambda(lambda mask: expand2square_mask(mask)),  # Pad to square
            Lambda(lambda mask: resize_mask(mask)),         # Resize
        ])
    else:
        transform = Compose([
            Lambda(lambda mask: resize_mask(mask)),         # Resize
...
def build_resize_transform(input_size, pad2square=False):
    if pad2square:
        transform = T.Compose([
            T.Resize((input_size, input_size), interpolation=InterpolationMode.BICUBIC),
        ])
        ...
        transform = T.Compose([
            T.Lambda(lambda img: T.Resize(
                (random.randint(int(0.5 * img.height), img.height),
                 random.randint(int(0.5 * img.width), img.width)),
...
def preprocess_mugcap(
        template_name,
        sources,
        tokenizer: transformers.PreTrainedTokenizer,
        num_image_token_list: list,
        num_mask_token_list: list,
        text_only: bool = False,
        group_by_length: bool = False,
        use_packed_ds: bool = False,
        ds_name: str = None,
    ...
    # Conduct when conversation starts with 'system'
    if conversations[0]['from'] == 'system':
        system_prompt = conversations[0]['value']
        conversations = conversations[1:]  # remove system prompt
    else:
    ...
    if not text_only:
        new_conversations = []
        current_image_idx, current_mask_idx = 0, 0
        for conversation in conversations:
            if conversation['from'] == 'human':
                # Image token add
                image_cnt = conversation['value'].count('<image>') # should be 1
                for i in range(image_cnt):
                    if current_image_idx == num_image:
                        break
                    image_tokens = f'{IMG_START_TOKEN}{IMG_CONTEXT_TOKEN * num_image_token_list[current_image_idx]}{IMG_END_TOKEN}'
                    ...
                    current_image_idx += 1
                # Masks token add
                mask_cnt = conversation['value'].count('<mask>')  # should be 1
                for i in range(mask_cnt):
                    if current_mask_idx == num_mask:
                        break
                    mask_tokens = f"{MASK_START_TOKEN}{MASK_CONTEXT_TOKEN * num_mask_token_list[current_mask_idx]}{MASK_END_TOKEN}"
                    conversation['value'] = conversation['value'].replace('<mask>', mask_tokens, 1)
                    current_mask_idx += 1
...
def decode_coco_rle(rle_mask):
    """
    Decode a COCO RLE mask into a binary mask.

    Args:
        rle_mask (dict): A COCO RLE mask with 'size' and 'counts' keys.

    Returns:
        np.ndarray: Decoded binary mask (dtype=uint8) with shape specified in the RLE.
    """
    if not ("size" in rle_mask and "counts" in rle_mask):
        raise ValueError("Invalid RLE format. The RLE mask must contain 'size' and 'counts' keys.")
