...
'''
(Pdb) first.keys()
dict_keys(['input_ids', 'labels', 'attention_mask', 'position_ids', 'pixel_values', 'image_flags'])
(Pdb) first['input_ids'].shape
torch.Size([1975])
(Pdb) first['labels'].shape
torch.Size([1975])
'''

def concat_pad_data_collator(features, max_item_length=None, pad_id=0):
    # Keys that are not used in the model forward would be deleted on train_dataloader_path.py.
    first = features[0]
    batch = {}
    ...
    for idx in range(len(features)):
        feat = features[idx]
        temp_input_ids = torch.LongTensor([pad_id] * max_item_length)
        temp_input_ids[:feat['input_ids'].shape[0]] = feat['input_ids'] # Padding
        feat['input_ids'] = temp_input_ids
        temp_labels = torch.LongTensor([IGNORE_INDEX] * max_item_length)
        temp_labels[:feat['labels'].shape[0]] = feat['labels']
        feat['labels'] = temp_labels
    ...
    # Again, we will use the first element to figure out which key/values are not None for this model.
    for k, v in first.items():
        if k not in ('label', 'label_ids', 'pixel_values', 'image_flags', 'mask_values') and \
            ...
            if isinstance(v, torch.Tensor):
                ...
                batch[k] = torch.tensor([f[k] for f in features])
        if k in ('pixel_values', 'image_flags', 'mask_values'):
            if isinstance(v, torch.Tensor):
